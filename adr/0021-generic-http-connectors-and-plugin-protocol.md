...
- **Slice 4 — `cron`**: shipped (v0.4 sprint 2). Two scheduling forms: `every_secs: N` (fixed interval, first emit one interval after start) and `at: "HH:MM"` (daily-at-time, local timezone). Synthesises `UserMessageReceived` with a fixed `content`; same `pipeline:` / `pipeline_input` glue as `http_inbound`. No network, ~250 LOC including tests.
