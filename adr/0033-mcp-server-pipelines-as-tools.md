...
## Addendum 2026-06-01 — 2b split: the sync slice is now buildable

A re-check of the external world changed the calculus the §6 decision rested on:

- **Claude Code shipped MCP elicitation** (~2.1.76, confirmed in official docs: *"Respond to MCP elicitation requests"* — Form/URL mode + an `Elicitation` hook). A concrete, widely-used elicitation-capable host now exists.
- **SEP-1686's elicitation-in-task rules are now `Final`** (spec *Task-augmented requests*: a request associated with a task MUST move it to `input_required` and MUST carry `related-task` in `_meta`). Those rules govern only the *async* path.

§6 conflated two things under "2b". They separate cleanly:

- **2b-sync (BUILT, this addendum):** when a pipeline hits an ADR-0022 approval during a **plain (non-task-augmented) `tools/call`**, the server issues `elicitation/create` *nested inside the live request* — correlated by JSON-RPC id only, with **no task and no `related-task`**. This still needs the transport to go **bidirectional + concurrent** (the server must initiate a request and the read loop must keep pumping to receive its response while the `tools/call` is parked on the approval bus), but it does **not** need the SEP-1686 task layer or `capabilities.tasks.requests.elicitation`. It only needs plain `capabilities.elicitation`, which Claude Code advertises. The §6 worry ("adoption thinner than async itself") was about the *task-correlated* elicitation; plain elicitation is broadly available.

- **2b-async (still PARKED):** elicitation correlated to a **backgrounded SEP-1686 task** via `related-task` (the `input_required` flow atop Slice 2a). No host is confirmed to drive elicitation against a background task; Claude Code's docs treat elicitation and tasks as independent features. Green-light unchanged: observe a host send `elicitation/create` carrying `related-task` against a task-augmented `tools/call`.

**What 2b-sync implements.** `zymi mcp serve` now starts approval channels on the runtime bus (previously it started none, so any approval gate fail-closed to a hard deny). A new `McpElicitationApprovalChannel` (`src/mcp/server/elicitation.rs`) bridges `ApprovalRequested` → `elicitation/create` (an approve/deny enum form) → `ApprovalGranted`/`ApprovalDenied`. Transport rework (`src/mcp/server/mod.rs` + `link.rs`): a single writer task drains an outbound mpsc, inbound handlers are spawned so the read loop keeps reading, and responses to server-initiated requests are routed by id through `McpClientLink`. Approval routing under serve is **auto + respects `project.yml`**: the elicitation channel is the default route only when the project declared no `default_approval_channel:`, and any project-declared channels (telegram/http) start alongside. A client without `capabilities.elicitation` makes the channel fail-closed with `ApprovalDenied { reason: "client_no_elicitation" }`.
